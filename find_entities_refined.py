...
def generate_span_annotations(
    ...
    rprint(f"[bold blue]Loading Refined model")
    ...
    for parquet_file in parquet_files:
        df = pd.read_parquet(parquet_file, columns=["document_id", "text_block.text", "text_block.language", "text_block.text_block_id", "text_block.type", "document_metadata.languages"])
        df_list.append(df)
    
    documents = pd.concat(df_list, ignore_index=True)
    
    rprint("[bold blue]Processing and filtering dataset...")
    documents = documents[~documents["text_block.type"].isin(["pageNumber", "pageHeader", "pageFooter"])]
    ...
    documents = documents[documents["document_metadata.languages"].apply(lambda x: "English" in x and len(x) == 1)]

    memory_usage = documents.memory_usage(deep=True).sum() / (1024 * 1024)
    rprint(f"[bold green]Dataset size: {len(documents)} rows, {memory_usage:.2f} MB in memory")
    ...
    with Progress(
        ...
        for document_id, document_df in document_groups:
            if (
                ...
                and Path(f"./data/{document_id}.json").exists()
            ):
                progress.update(task, advance=1, description=f"Skipped {document_id}")
                continue
            
            progress.update(task, description=f"Processing {document_id} ({len(document_df)} text blocks)")
                    ...
                    if span.predicted_entity is not None:
                        ...
                        passages[text].spans.append(
                            Span(
                                ...
                                fine_grained_type=span.predicted_entity_types[0][1] if span.predicted_entity_types else None,
                                id=span.predicted_entity.wikidata_entity_id or "unknown",
